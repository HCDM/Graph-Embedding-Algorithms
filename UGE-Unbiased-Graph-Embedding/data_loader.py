...
def process_raw_pokec(raw_folder, processed_folder, data_name):
    ...
    if data_name == 'pokec-z':
        ...
        sensitive_attributes_predefined = SENSITIVE_ATTR_DICT['pokec-z']
        ...
        sensitive_attributes_predefined = SENSITIVE_ATTR_DICT['pokec-n']
        
    else:
        raise Exception('unknown pokec dataset {}'.format(data_name))
    ...
    node_attributes = nodes.filter(sensitive_attributes_predefined.append('AGE')) # note that in original data, column name for age is 'AGE'

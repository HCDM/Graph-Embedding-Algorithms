...
def load_user_attributes_movielens(file, M):
    ...
    with open(file, 'r') as fin:
        ...
        for line in lines:
            ...
            if gender not in gender_group:
                gender_group[gender] = []
            gender_group[gender].append(user_idx)

            if age not in age_group:
                age_group[age] = []
            age_group[age].append(user_idx)
            
            if occupation not in occupation_group:
                occupation_group[occupation] = []
            occupation_group[occupation].append(user_idx)

    return user_attributes, {'gender':gender_group, 'occupation':occupation_group, 'age':age_group}
...
def eval_unbiasedness_movielens(data_name, embed_file=None):
    ...
    user_attributes, attr_groups = load_user_attributes_movielens('{}/ml-1m/users.dat'.format(RAW_FOLDER), M)
